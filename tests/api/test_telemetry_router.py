...
# ── Cache hit tests — cover return cached paths ───────────────────────────────


@pytest.mark.asyncio
async def test_get_global_stats_cache_hit(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/stats/global returns cached value without calling planner."""
    planner = Mock()
    planner.get_snapshot = AsyncMock()

    cached_result = {"global_state": "OK", "total_nodes": 10}
    svc_cache = ServiceCache()
    await svc_cache.set("stats:global", cached_result, ttl=60.0)
...
@pytest.mark.asyncio
async def test_get_room_state_cache_hit(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/rooms/{room_id}/state returns cached value without calling planner."""
    planner = Mock()
    planner.get_snapshot = AsyncMock()

    cached_result = {"room_id": "room1", "state": "OK", "racks": {}}
    svc_cache = ServiceCache()
    await svc_cache.set("room:room1:state", cached_result, ttl=60.0)
...
@pytest.mark.asyncio
async def test_get_rack_state_cache_hit(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/racks/{rack_id}/state returns cached value without calling planner."""
    planner = Mock()
    planner.get_snapshot = AsyncMock()

    cached_result = {"rack_id": "rack01", "state": "OK", "devices": []}
    svc_cache = ServiceCache()
    await svc_cache.set("rack:rack01:state", cached_result, ttl=60.0)
...
# ── get_global_stats WARN path ─────────────────────────────────────────────────


@pytest.mark.asyncio
async def test_get_global_stats_warn_status(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/stats/global returns WARN when some racks are WARN."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(
        return_value=PlannerSnapshot(
            generated_at=1000000.0,
            rack_states={"rack01": "WARN"},
            node_states={"node01": "WARN", "node02": "OK"},
            node_alerts={"node01": {"node_up": "WARN"}},
        )
    )
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["status"] in ("WARN", "CRIT", "OK", "UNKNOWN")
...
@pytest.mark.asyncio
async def test_get_all_room_states_success(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/rooms/states returns dict of room_id → health state."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(
        return_value=PlannerSnapshot(
            generated_at=1000000.0,
    ...
    response = client.get("/api/rooms/states")
    assert response.status_code == 200
    data = response.json()
    assert "room1" in data
    assert data["room1"] in ("OK", "WARN", "CRIT", "UNKNOWN")

    app.dependency_overrides.clear()


@pytest.mark.asyncio
async def test_get_all_room_states_no_deps():
    """GET /api/rooms/states returns empty dict when topology not loaded."""
    app.dependency_overrides[get_topology_optional] = lambda: None
    app.dependency_overrides[get_catalog_optional] = lambda: None
    app.dependency_overrides[get_checks_library_optional] = lambda: None
    app.dependency_overrides[get_planner_optional] = lambda: None
...
@pytest.mark.asyncio
async def test_get_all_room_states_cache_hit(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/rooms/states returns cached value without calling planner."""
    planner = Mock()
    planner.get_snapshot = AsyncMock()

    cached = {"room1": "CRIT", "room2": "OK"}
    svc_cache = ServiceCache()
    await svc_cache.set("rooms:states", cached, ttl=60.0)
...
@pytest.mark.asyncio
async def test_get_room_state_warn_propagates(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/rooms/{room_id}/state propagates WARN state from racks."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(
        return_value=PlannerSnapshot(
            generated_at=1000000.0,
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["state"] in ("WARN", "CRIT")

    app.dependency_overrides.clear()


# Client that suppresses server exceptions (for testing 500 paths)
_no_raise_client = TestClient(app, raise_server_exceptions=False)
# ── Exception paths — svc_cache.cancel_inflight ───────────────────────────────


@pytest.mark.asyncio
async def test_get_global_stats_planner_exception(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/stats/global returns 500 when planner raises, cancels inflight."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(side_effect=RuntimeError("planner crash"))
    ...
    response = _no_raise_client.get("/api/stats/global")
...
@pytest.mark.asyncio
async def test_get_room_state_planner_exception(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/rooms/{room_id}/state returns 500 when planner raises."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(side_effect=RuntimeError("planner crash"))
    ...
    response = _no_raise_client.get("/api/rooms/room1/state")
    assert response.status_code == 500
...
@pytest.mark.asyncio
async def test_get_all_room_states_planner_exception(
    mock_topology, mock_catalog, mock_checks_library
):
    """GET /api/rooms/states returns 500 when planner raises."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(side_effect=RuntimeError("planner crash"))
    ...
    response = _no_raise_client.get("/api/rooms/states")
    assert response.status_code == 500
...
@pytest.mark.asyncio
async def test_get_rack_state_planner_exception(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/racks/{rack_id}/state returns 500 when planner raises."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(side_effect=RuntimeError("planner crash"))
    ...
    response = _no_raise_client.get("/api/racks/rack01/state")
    assert response.status_code == 500

    app.dependency_overrides.clear()


# ── get_all_room_states CRIT/WARN propagation ─────────────────────────────────


@pytest.mark.asyncio
async def test_get_all_room_states_crit_status(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/rooms/states returns CRIT when a rack is critical."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(
        return_value=PlannerSnapshot(
            generated_at=1000000.0,
            rack_states={"rack01": "CRIT"},
            node_states={"node01": "CRIT"},
            node_alerts={"node01": {"node_up": "CRIT"}},
        )
    )
    ...
    assert response.status_code == 200
    data = response.json()
    assert data.get("room1") == "CRIT"

    app.dependency_overrides.clear()


@pytest.mark.asyncio
async def test_get_all_room_states_warn_status(mock_topology, mock_catalog, mock_checks_library):
    """GET /api/rooms/states returns WARN when a rack is WARN but none CRIT."""
    planner = Mock()
    planner.get_snapshot = AsyncMock(
        return_value=PlannerSnapshot(
            generated_at=1000000.0,
