...
# ── TelemetryPlanner.update_config ────────────────────────────────────────────


def test_update_config_resets_snapshot():
    """update_config replaces config and clears cached snapshot."""
    planner = TelemetryPlanner(PlannerConfig(cache_ttl_seconds=60))
    from rackscope.telemetry.planner import PlannerSnapshot

    planner._snapshot = PlannerSnapshot(
        generated_at=9999999.0,
        rack_states={},
        node_states={},
        node_alerts={},
    )
    new_config = PlannerConfig(cache_ttl_seconds=30)
    planner.update_config(new_config)
    assert planner.config.cache_ttl_seconds == 30
    assert planner._snapshot is None


# ── get_snapshot cache hit (TTL not expired) ──────────────────────────────────


@pytest.mark.asyncio
async def test_get_snapshot_returns_cached_when_fresh():
    """get_snapshot returns the cached snapshot without recomputing when TTL valid."""
    import time

    planner = TelemetryPlanner(PlannerConfig(cache_ttl_seconds=3600))
    from rackscope.telemetry.planner import PlannerSnapshot

    cached = PlannerSnapshot(
        generated_at=time.monotonic(),
        rack_states={"rack01": "OK"},
        node_states={},
        node_alerts={},
    )
    planner._snapshot = cached

    topology = _make_topology()
    checks = ChecksLibrary(checks=[])
    result = await planner.get_snapshot(topology, checks, {})
    assert result is cached


@pytest.mark.asyncio
async def test_get_snapshot_concurrent_requests_use_cached():
    """Concurrent get_snapshot calls return the same snapshot after first computes."""
    import time

    planner = TelemetryPlanner(PlannerConfig(cache_ttl_seconds=3600))
    from rackscope.telemetry.planner import PlannerSnapshot

    fresh = PlannerSnapshot(
        generated_at=time.monotonic(),
        rack_states={},
        node_states={},
        node_alerts={},
    ...
    async def _mock_recompute(*args, **kwargs):
        planner._snapshot = fresh
        return fresh

    topology = _make_topology()
    checks = ChecksLibrary(checks=[])

    with patch.object(planner, "_recompute", side_effect=_mock_recompute):
        r1, r2 = await asyncio.gather(
            planner.get_snapshot(topology, checks, {}),
            planner.get_snapshot(topology, checks, {}),
        )
