    ...
    <div className="flex h-32 items-end gap-2">
      {data.map((val, i) => (
        <div key={i} className="flex flex-1 flex-col items-center gap-1">
          <div className={`w-full rounded-t-sm ${color}`} style={{ height: `${(val / max) * 100}%` }} />
          <span className="text-[9px] text-gray-400">{['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug'][i]}</span>
        </div>
      ))}
    </div>
  );
      ...
      <svg viewBox={`0 0 ${w} ${h}`} className="h-full w-full" preserveAspectRatio="none">
        <defs>
          <linearGradient id={`grad-${color}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={color === 'brand' ? '#465fff' : '#12b76a'} stopOpacity="0.3" />
            <stop offset="100%" stopColor={color === 'brand' ? '#465fff' : '#12b76a'} stopOpacity="0.02" />
          </linearGradient>
        </defs>
        <polygon points={fill} fill={`url(#grad-${color})`} />
        <polyline points={pts} fill="none" stroke={color === 'brand' ? '#465fff' : '#12b76a'} strokeWidth="2" />
      </svg>
    </div>
  );
};
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <SectionCard title="Line Chart" desc="Trend visualization over time">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="rounded-full bg-success-50 px-2.5 py-1 text-xs font-medium text-success-500 dark:bg-success-500/10">+12.5%</span>
        </div>
        <SimpleLineChart color="brand" />
        <div className="mt-3 flex gap-2">
          {['7d', '1m', '3m', '1y'].map((p, i) => (
            <button key={p} className={`rounded px-2.5 py-1 text-xs font-medium ${i === 1 ? 'bg-brand-500 text-white' : 'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-white/5'}`}>{p}</button>
          ))}
        </div>
      </SectionCard>
      <SectionCard title="Area Chart" desc="Filled area under the line">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="rounded-full bg-success-50 px-2.5 py-1 text-xs font-medium text-success-500 dark:bg-success-500/10">+8.2%</span>
        </div>
      ...
      <SectionCard title="Multi-Color Bar Chart" desc="Categories with distinct colors">
        <div className="mb-4">
          <div className="flex items-center gap-4 text-xs text-gray-500">
            ...
            <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-success-500" />Profit</span>
          </div>
        </div>
        <div className="flex h-32 items-end gap-2">
          {[65, 80, 55, 95, 70, 85, 60, 90].map((val, i) => (
            <div key={i} className="flex flex-1 flex-col items-center gap-0.5">
              <div className="flex w-full gap-0.5">
                <div className="flex-1 rounded-t-sm bg-brand-500" style={{ height: `${val * 0.9}px` }} />
                <div className="flex-1 rounded-t-sm bg-success-500" style={{ height: `${val * 0.5}px` }} />
              </div>
              <span className="text-[9px] text-gray-400">{['J','F','M','A','M','J','J','A'][i]}</span>
            </div>
          ))}
        </div>
      </SectionCard>
        ...
        <div className="flex items-center justify-center gap-8">
          <div className="relative h-32 w-32">
            <svg viewBox="0 0 36 36" className="h-full w-full -rotate-90">
              ...
              <circle cx="18" cy="18" r="15.9" fill="none" stroke="#465fff" strokeWidth="3" strokeDasharray="45 55" />
              <circle cx="18" cy="18" r="15.9" fill="none" stroke="#12b76a" strokeWidth="3" strokeDasharray="30 70" strokeDashoffset="-45" />
              <circle cx="18" cy="18" r="15.9" fill="none" stroke="#f79009" strokeWidth="3" strokeDasharray="18 82" strokeDashoffset="-75" />
            </svg>
          ...
          <div className="space-y-2">
            {[['Direct', '45%', 'bg-brand-500'], ['Organic', '30%', 'bg-success-500'], ['Paid', '18%', 'bg-warning-500'], ['Other', '7%', 'bg-gray-300']].map(([l, p, c]) => (
      ...
      <SectionCard title="Progress Radials" desc="Circular progress indicators">
        <div className="flex flex-wrap justify-around gap-4">
          {[{ val: 75, color: '#465fff', label: 'Progress' }, { val: 60, color: '#12b76a', label: 'Success' }, { val: 40, color: '#f79009', label: 'Warning' }].map(({ val, color, label }) => {
            ...
            return (
              <div key={label} className="flex flex-col items-center gap-2">
                <div className="relative h-20 w-20">
                  <svg viewBox="0 0 36 36" className="h-full w-full -rotate-90">
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="#e4e7ec" strokeWidth="3" />
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke={color} strokeWidth="3" strokeDasharray={`${dash} ${circ - dash}`} strokeLinecap="round" />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
