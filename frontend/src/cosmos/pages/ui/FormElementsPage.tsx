...
const inputBase = 'w-full rounded-lg border px-3.5 py-2.5 text-sm transition-colors focus:outline-none';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Input components for data collection</p>
      </div>
      <div className="grid gap-6 lg:grid-cols-2">
        <SectionCard title="Text Inputs" desc="Basic input field variations">
          <div className="space-y-4">
            <div><Label>Default input</Label><input type="text" placeholder="Enter text..." className={inputNormal} /></div>
            <div><Label>Disabled</Label><input type="text" placeholder="Disabled field" disabled className={`${inputNormal} cursor-not-allowed opacity-60`} /></div>
            <div><Label>With placeholder</Label><input type="text" placeholder="e.g. John Doe" className={inputNormal} /></div>
          </div>
        </SectionCard>
        <SectionCard title="Validation States" desc="Error, success, and default states">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-success-500">Looks good!</p>
            </div>
          </div>
        </SectionCard>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" placeholder="you@example.com" className={`${inputNormal} pl-10`} />
              </div>
            </div>
            <div>
              <Label>Phone</Label>
              <div className="relative">
                ...
                <input type="tel" placeholder="+1 (555) 000-0000" className={`${inputNormal} pl-10`} />
              </div>
            </div>
            <div>
              <Label>URL</Label>
              <div className="relative">
                ...
                <input type="url" placeholder="https://example.com" className={`${inputNormal} pl-10`} />
              </div>
            </div>
          </div>
        </SectionCard>
        <SectionCard title="Password Field" desc="Password with show/hide toggle">
          <div>
            ...
            <div className="relative">
              <input type={showPass ? 'text' : 'password'} placeholder="Enter your password" className={`${inputNormal} pr-10`} />
        ...
        <SectionCard title="Checkboxes & Radios" desc="Selection controls">
          <div className="space-y-4">
            <div>
              <p className="mb-2 text-xs font-medium text-gray-500 dark:text-gray-400">Checkboxes</p>
              {['Option A', 'Option B', 'Option C'].map((opt, i) => {
                const key = ['a', 'b', 'c'][i];
                return (
                  <label key={opt} className="mb-2 flex cursor-pointer items-center gap-3">
                    <input type="checkbox" checked={checks[key]} onChange={() => setChecks({ ...checks, [key]: !checks[key] })} className="h-4 w-4 rounded border-gray-300 accent-brand-500" />
            ...
            <div>
              <p className="mb-2 text-xs font-medium text-gray-500 dark:text-gray-400">Radio buttons</p>
              {['Option 1', 'Option 2', 'Option 3'].map((opt, i) => (
                <label key={opt} className="mb-2 flex cursor-pointer items-center gap-3">
                  <input type="radio" name="demo-radio" value={`opt${i + 1}`} checked={radio === `opt${i + 1}`} onChange={() => setRadio(`opt${i + 1}`)} className="h-4 w-4 accent-brand-500" />
                  <span className="text-sm text-gray-700 dark:text-gray-300">{opt}</span>
                </label>
              ))}
            </div>
          ...
          <div className="space-y-4">
            {[
              ...
              <label key={label} className="flex cursor-pointer items-center justify-between">
                ...
                <button onClick={() => set(!val)} className={`relative h-6 w-11 rounded-full transition-colors ${val ? 'bg-brand-500' : 'bg-gray-200 dark:bg-gray-700'}`}>
                  <span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition-transform ${val ? 'translate-x-5' : 'translate-x-1'}`} />
                </button>
              </label>
            ))}
          </div>
        ...
        <SectionCard title="File Upload" desc="File input with drag and drop style">
          <div className="rounded-xl border-2 border-dashed border-gray-200 p-8 text-center dark:border-gray-700">
            ...
            <p className="mt-3 text-sm font-medium text-gray-700 dark:text-gray-300">Drag & drop files here</p>
            <p className="mt-1 text-xs text-gray-400">PNG, JPG, PDF up to 10MB</p>
            <label className="mt-4 inline-flex cursor-pointer items-center gap-2 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">
