...
const Sk = ({ h = 'h-4', w = 'w-full', round = 'rounded' }: { h?: string; w?: string; round?: string }) => (
  <div className={`animate-pulse bg-gray-200 dark:bg-gray-700 ${h} ${w} ${round}`} />
);

export const SkeletonPage = () => (
  <div className="space-y-6">
    <div><h2 className="text-xl font-bold text-gray-900 dark:text-white">Skeleton Loader</h2>
      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Animated loading placeholder components</p></div>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800">
          <div className="border-b border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-800">
            <div className="grid grid-cols-4 gap-4">
              {Array.from({ length: 4 }).map((_, i) => <Sk key={i} h="h-3" w="w-3/4" />)}
            </div>
          </div>
          {Array.from({ length: 4 }).map((_, row) => (
            <div key={row} className="border-b border-gray-100 px-4 py-3 last:border-0 dark:border-gray-800">
              <div className="grid grid-cols-4 gap-4">
                {Array.from({ length: 4 }).map((_, col) => <Sk key={col} h="h-3" w={col === 0 ? 'w-full' : col === 3 ? 'w-1/2' : 'w-5/6'} />)}
              </div>
            </div>
          ))}
        </div>
