...
import type { CheckDefinition, DeviceTemplate, RackTemplate } from '../../../types';
...
const SEVERITY_COLORS: Record<string, { bg: string; text: string; border: string; light: string }> = {
  OK:      { bg: '#10b981', text: '#fff', border: '#10b981', light: 'bg-green-50 text-green-700 border-green-200 dark:bg-green-500/10 dark:text-green-400 dark:border-green-500/30' },
  WARN:    { bg: '#f59e0b', text: '#fff', border: '#f59e0b', light: 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/30' },
  CRIT:    { bg: '#ef4444', text: '#fff', border: '#ef4444', light: 'bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/30' },
  UNKNOWN: { bg: '#6b7280', text: '#fff', border: '#6b7280', light: 'bg-gray-50 text-gray-600 border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700' },
};
...
const SCOPE_BADGES: Record<string, string> = {
  ...
  chassis: 'bg-purple-50 text-purple-600 border-purple-200 dark:bg-purple-500/10 dark:text-purple-400 dark:border-purple-500/30',
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-100 dark:border-gray-800">
        {rules.length === 0 ? (
          ...
          <div>
            <div className="grid grid-cols-3 border-b border-gray-100 bg-gray-50 px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:border-gray-800 dark:bg-gray-800/50 dark:text-gray-600">
              <span>Operator</span><span>Value</span><span>Severity</span>
            </div>
            {rules.map((rule, i) => {
              ...
              return (
                ...
                >
                  <span className="font-mono text-xs font-bold text-gray-700 dark:text-gray-300">{rule.op}</span>
                  <span className="font-mono text-xs text-gray-700 dark:text-gray-300">{String(rule.value)}</span>
      ...
      {isBool && boolStates.length > 0 && (
        <div>
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-600">State mapping</p>
          <div className="flex gap-2">
            {boolStates.map(({ value, severity }) => {
              ...
              return (
                ...
                >
                  <span className="font-mono text-lg font-black" style={{ color: s.border }}>{value}</span>
      ...
      {isNumeric && rules.some((r) => ['>', '>=', '<', '<='].includes(r.op)) && (
        <div>
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-600">Threshold overview</p>
              ...
              .map((r, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2">
                    <div className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: s.border }} />
    ...
    <div>
      ...
      {total === 0 ? (
        ...
        <div className="space-y-1">
          {devices.map((t) => (
            <div key={t.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs hover:bg-gray-50 dark:hover:bg-white/5">
              <Tag className="h-3 w-3 shrink-0 text-blue-400" />
              <span className="flex-1 truncate text-gray-700 dark:text-gray-300">{t.name}</span>
              <span className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-[9px] text-gray-500 dark:bg-gray-800 dark:text-gray-500">device</span>
            </div>
          ))}
          {racks.map((t) => (
            <div key={t.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs hover:bg-gray-50 dark:hover:bg-white/5">
              ...
              <span className="flex-1 truncate text-gray-700 dark:text-gray-300">{t.name}</span>
              <span className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-[9px] text-gray-500 dark:bg-gray-800 dark:text-gray-500">rack</span>
            </div>
          ))}
        </div>
      )}
...
const EditorPanel = ({
  ...
  const update = useCallback(<K extends keyof CheckDraft>(key: K, value: CheckDraft[K]) => {
    setDraft((d) => {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="shrink-0 flex items-center justify-between px-5 py-3.5">
        ...
        <div className="flex shrink-0 items-center gap-2 ml-3">
          ...
          {dirty && (
            <button
              onClick={() => { const d = toDraft(check); setDraft(d); setDirty(false); onDraftChange?.(d); }}
      ...
      <div className="flex-1 space-y-5 overflow-y-auto px-5 pb-5">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={draft.name} onChange={(e) => update('name', e.target.value)} placeholder="Node up" className={inputCls} />
              </div>
              <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input value={draft.kind} onChange={(e) => update('kind', e.target.value)} placeholder="ipmi, pdu, core…" className={inputCls} />
              </div>
              <div>
                <label className={labelCls}>Scope</label>
                <select value={draft.scope} onChange={(e) => update('scope', e.target.value)} className={inputCls}>
                  {SCOPES.map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
              <div>
                <label className={labelCls}>Output</label>
                <select value={draft.output} onChange={(e) => update('output', e.target.value)} className={inputCls}>
        ...
        <SectionCard title="PromQL Expression">
          <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700" style={{ height: 140 }}>
            <MonacoEditor
              height={140}
        ...
        <SectionCard title="Threshold Rules" desc="Define when the check triggers WARN/CRIT/OK">
          <div className="space-y-2">
            {draft.rules.map((rule, i) => {
              const s = SEVERITY_COLORS[rule.severity] ?? SEVERITY_COLORS.UNKNOWN;
              return (
                <div key={i} className="flex items-center gap-2 rounded-xl border border-gray-100 bg-gray-50 px-3 py-2 dark:border-gray-800 dark:bg-gray-800/40"
                  style={{ borderLeft: `3px solid ${s.border}` }}>
            ...
            <button onClick={addRule}
              className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-dashed border-gray-200 py-2 text-xs font-medium text-gray-400 transition-colors hover:border-brand-300 hover:text-brand-500 dark:border-gray-700 dark:hover:border-brand-600 dark:hover:text-brand-400">
...
const NewCheckModal = ({
  ...
  const [form, setForm] = useState({
    name: '', id: '', kind: '', scope: 'node', output: 'bool', expr: '',
    group: fileList[0] ?? '', newGroup: '',
  });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const autoId = form.name.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '').replace(/_+/g, '_').replace(/^_|_$/g, '');
  ...
  const handleCreate = async () => {
    ...
    if (!effectiveId) { setError('ID is required'); return; }
    if (existingIds.includes(effectiveId)) { setError('ID already exists'); return; }
    if (useNewGroup && !form.newGroup.trim()) { setError('Group name is required'); return; }
    setSaving(true); setError(null);
    try {
      await onCreated({
        id: effectiveId, name: form.name.trim(), kind: form.kind || form.group.replace('.yaml', ''),
        scope: form.scope, output: form.output, expr: form.expr.trim(),
      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900">
        ...
        <div className="mt-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelCls}>Name *</label>
              <input autoFocus value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Node up" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>ID <span className="text-gray-400">(auto)</span></label>
              <input value={form.id} onChange={(e) => setForm((f) => ({ ...f, id: e.target.value }))} placeholder={autoId || 'check_id'} className={`${inputCls} font-mono text-xs`} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div>
            ...
            <div>
              <label className={labelCls}>Output</label>
              <select value={form.output} onChange={(e) => setForm((f) => ({ ...f, output: e.target.value }))} className={inputCls}>
                {OUTPUTS.map((o) => <option key={o} value={o}>{o}</option>)}
              </select>
            </div>
            <div>
              <label className={labelCls}>Kind <span className="text-gray-400">(opt.)</span></label>
              <input value={form.kind} onChange={(e) => setForm((f) => ({ ...f, kind: e.target.value }))} placeholder="ipmi, core…" className={inputCls} />
            </div>
          </div>
          {/* Group selection */}
          <div>
            <label className={labelCls}>Group (file)</label>
            <select value={form.group} onChange={(e) => setForm((f) => ({ ...f, group: e.target.value }))} className={inputCls}>
              {fileList.map((f) => <option key={f} value={f}>{f.replace('.yaml', '')}</option>)}
          ...
          {useNewGroup && (
            <div>
              ...
              <input value={form.newGroup} onChange={(e) => setForm((f) => ({ ...f, newGroup: e.target.value }))} placeholder="my-checks" className={`${inputCls} font-mono text-xs`} />
              {form.newGroup && <p className="mt-1 text-[10px] text-gray-400">Will create: <span className="font-mono">{form.newGroup.replace(/[^a-z0-9-_]/g, '')}.yaml</span></p>}
            </div>
          )}
...
const DeleteConfirmModal = ({ name, onConfirm, onCancel }: { name: string; onConfirm: () => void; onCancel: () => void }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
    <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900">
      <button onClick={onCancel} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><X className="h-5 w-5" /></button>
      ...
      <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
        You are about to permanently delete <span className="font-semibold text-gray-700 dark:text-gray-300">{name}</span>. This will remove it from its YAML file and all templates that reference it will lose this check.
      </p>
      <div className="mt-6 flex justify-end gap-3">
        <button onClick={onCancel} className="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">Cancel</button>
        <button onClick={onConfirm} className="rounded-lg bg-red-500 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-red-600">Delete</button>
      </div>
    </div>
  </div>
);
...
export const CosmosChecksEditorPage = () => {
    ...
    try {
      ...
      fileResults.forEach((data: { name: string; content: string }, i) => {
        ...
        try {
          ...
          checks.forEach((c) => { if (c.id) manifest[c.id] = name; });
  ...
  const grouped = useMemo(() => {
    const filtered = allChecks.filter(
      (c) => !search || c.name.toLowerCase().includes(search.toLowerCase()) || c.id.toLowerCase().includes(search.toLowerCase())
    );
  ...
  const handleSaveCheck = async (draft: CheckDraft) => {
    ...
    try { parsed = (jsYaml.load(content) as Record<string, unknown>) ?? { checks: [] }; }
    catch { parsed = { checks: [] }; }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col space-y-5">
      <PageHeader
        ...
        breadcrumb={<PageBreadcrumb items={[{ label: 'Home', href: '/cosmos' }, { label: 'Editors' }, { label: 'Checks Library' }]} />}
        actions={
          !loading && !loadError ? (
          ...
          <div className="flex w-80 shrink-0 flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
            <div className="shrink-0 p-3">
              <div className="relative">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search checks…"
              ...
              {grouped.length === 0 ? (
                ...
                <div className="space-y-1 p-2">
                  {grouped.map(([kind, checks]) => {
                    ...
                    return (
                      <div key={kind} className="overflow-hidden rounded-xl border border-gray-100 dark:border-gray-800">
                        <button
                          onClick={() => setOpenKinds((prev) => {
                            const next = new Set(prev);
                            if (isOpen) { next.delete(kind); } else { next.add(kind); }
                            return next;
                          })}
                        ...
                        >
                          ...
                          <span className="flex-1 text-xs font-semibold capitalize text-gray-700 dark:text-gray-300">{kind}</span>
                          <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-500 dark:bg-gray-800 dark:text-gray-400">{checks.length}</span>
                          ...
                          <div className="space-y-0.5 border-t border-gray-100 p-1 dark:border-gray-800">
                            {checks.map((check) => {
                              ...
                              return (
                                ...
                                >
                                  <span className={`text-xs font-semibold ${selected ? 'text-brand-600 dark:text-brand-400' : 'text-gray-700 dark:text-gray-300'}`}>
                                    {check.name}
                                  </span>
                                  <div className="flex items-center gap-1.5">
                                    <span className="font-mono text-[10px] text-gray-400 dark:text-gray-600">{check.id}</span>
                                    <span className={`rounded-full border px-1.5 py-0 text-[9px] font-semibold ${SCOPE_BADGES[check.scope] ?? SCOPE_BADGES.node}`}>{check.scope}</span>
                                    {check.output && (
                                      <span className="rounded-full bg-gray-100 px-1.5 py-0 font-mono text-[9px] text-gray-500 dark:bg-gray-800 dark:text-gray-500">{check.output}</span>
                                    )}
                                  </div>
                                </button>
                              );
            ...
            <div className="shrink-0 border-t border-gray-100 px-4 py-2.5 dark:border-gray-800">
              <p className="text-[10px] text-gray-400 dark:text-gray-600">{allChecks.length} check{allChecks.length !== 1 ? 's' : ''} · {fileList.length} file{fileList.length !== 1 ? 's' : ''}</p>
            </div>
          </div>
            ...
            {selectedCheck ? (
              ...
              <div className="flex h-full items-center justify-center p-6 text-center">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Select a check to edit</p>
            ...
            {previewCheck ? (
              <div className="flex h-full flex-col">
                ...
                <div className="shrink-0 border-b border-gray-100 px-5 py-3.5 dark:border-gray-800">
                  <div className="flex items-center gap-2">
                    <div className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: kindColor(previewCheck.kind ?? '') }} />
                    <p className="text-sm font-bold text-gray-800 dark:text-white">{previewCheck.name}</p>
                    <span className={`ml-auto rounded-full border px-2 py-0.5 text-[10px] font-semibold ${SCOPE_BADGES[previewCheck.scope] ?? SCOPE_BADGES.node}`}>
                      {previewCheck.scope}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-6 overflow-y-auto p-5">
                  {/* Rules visualization */}
                  <div>
                    <p className="mb-3 text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-600">Rules</p>
                    <RuleVisualizer check={previewCheck} />
                  </div>

                  {/* Usage cross-reference */}
                  <div>
                    <UsagePanel checkId={previewCheck.id} deviceTemplates={deviceTemplates} rackTemplates={rackTemplates} />
                  </div>

                  {/* File location */}
                  {fileManifest[previewCheck.id] && (
                    <div>
                      <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-600">File</p>
              ...
              <div className="flex h-full items-center justify-center p-6 text-center">
                <div>
                  ...
                  <p className="text-sm text-gray-400 dark:text-gray-600">Select a check to see its rules and usage</p>
                </div>
              </div>
            )}
          </div>
