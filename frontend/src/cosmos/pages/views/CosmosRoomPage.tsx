...
const RackCard = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative flex flex-col items-center gap-1.5 rounded-xl border-2 p-3 transition-all hover:shadow-lg ${selected ? 'ring-2 ring-brand-500 ring-offset-2 dark:ring-offset-gray-950' : ''}`}
    ...
    >
      ...
      <span className="font-mono text-xs font-semibold text-gray-700 dark:text-gray-300 truncate max-w-full">{rack.id}</span>
      <span className={`rounded px-1.5 py-0.5 text-[9px] font-bold uppercase ${healthBg[state]} ${healthText[state]}`}>{state}</span>
      {metrics && (
...
export const CosmosRoomPage = () => {
  ...
  useEffect(() => {
    ...
    let active = true;
    api.getRackState(selectedRack.id, false)
      .then((data) => { if (active) setSelectedHealth(data); })
      .catch(() => {});
    return () => { active = false; };
  }, [selectedRack]);
  ...
  if (!room) return <div className="flex h-64 items-center justify-center text-gray-400">Room not found</div>;
  ...
  const summary = allRacks.reduce((acc, rack) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={loadHealth} className="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-white/5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[['OK','#10b981'],['WARN','#f59e0b'],['CRIT','#ef4444'],['UNKNOWN','#6b7280']].map(([state, color]) => (
          <div key={state} className="rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900">
            <div className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: color }} />
              <span className="text-xs font-semibold text-gray-500 dark:text-gray-400">{state}</span>
            </div>
            <p className="mt-1 text-xl font-bold text-gray-900 dark:text-white">{summary[state] ?? 0}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr,340px]">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
          <h3 className="mb-4 text-sm font-semibold text-gray-700 dark:text-gray-300">Floor Plan</h3>
          {room.aisles && room.aisles.length > 0 ? (
              ...
              {room.aisles.map((aisle) => (
                <div key={aisle.id}>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span className="text-xs font-semibold uppercase tracking-wider text-gray-400">{aisle.name}</span>
                  </div>
            ...
            <div className="flex h-32 items-center justify-center text-sm text-gray-400">No aisles configured</div>
          )}
          {(room.standalone_racks ?? []).length > 0 && (
            <div className="mt-6">
              <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">Standalone Racks</div>
              <div className="flex flex-wrap gap-3">
                {(room.standalone_racks ?? []).map((rack) => (
                  <div key={rack.id} className="w-20">
                    <RackCard rack={rack} health={healthMap[rack.id]} selected={selectedRack?.id === rack.id} onClick={() => setSelectedRack(rack)} />
                  </div>
                ))}
              </div>
            </div>
          ...
          {!selectedRack ? (
            ...
            <div className="p-5">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white">{selectedRack.name}</h3>
              ...
              {selectedHealth && (
                <>
                  {/* Health badge */}
                  <div className={`mb-4 inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold ${healthBg[selectedHealth.state]} ${healthText[selectedHealth.state]}`}>
                    <span className="h-2 w-2 rounded-full" style={{ backgroundColor: healthColor[selectedHealth.state] }} />
                    ...
                    <div className="mb-4 grid grid-cols-2 gap-3">
                      {selectedHealth.metrics.temperature > 0 && (
                        <div className="rounded-lg bg-gray-50 p-3 dark:bg-gray-800">
                          ...
                          <p className="mt-1 text-lg font-bold text-gray-900 dark:text-white">{Math.round(selectedHealth.metrics.temperature)}°C</p>
                        </div>
                      )}
                      {selectedHealth.metrics.power > 0 && (
                        <div className="rounded-lg bg-gray-50 p-3 dark:bg-gray-800">
                          <div className="flex items-center gap-1.5 text-xs text-gray-500"><Zap className="h-3.5 w-3.5" />Power</div>
                          <p className="mt-1 text-lg font-bold text-gray-900 dark:text-white">{(selectedHealth.metrics.power / 1000).toFixed(1)} kW</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {selectedHealth.nodes && Object.keys(selectedHealth.nodes).length > 0 && (
                    <div>
                      <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">Nodes ({Object.keys(selectedHealth.nodes).length})</p>
                      <div className="grid grid-cols-5 gap-1">
                        {Object.entries(selectedHealth.nodes).slice(0, 20).map(([node, ns]) => {
                          const s = (ns as { state?: string }).state ?? 'UNKNOWN';
                          return (
                            <div key={node} title={`${node}: ${s}`} className="aspect-square rounded" style={{ backgroundColor: healthColor[s] ?? healthColor.UNKNOWN, opacity: 0.7 }} />
                          );
                        })}
                      </div>
                    </div>
