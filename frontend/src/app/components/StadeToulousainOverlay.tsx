...
// ── Stade Toulousain SVG logo (inline — no external URL, CSP-safe) ─────────────
// Source: https://upload.wikimedia.org/wikipedia/fr/0/01/Logo_Stade_Toulousain_Rugby.svg
// License: Wikipedia / free use for identification purposes
...
// ── Bilingual text ──────────────────────────────────────────────────────────────
type Lang = 'fr' | 'en';
type BodyItem = { icon: string; title: string; text: string };
type LangContent = {
  alert: string;
  subtitle: string;
  body: BodyItem[];
...
const TEXT: Record<Lang, LangContent> = {
  fr: {
    alert: '🔴⚫ Alerte : Vérité Scientifique (et un peu chauvine)',
    subtitle: "Félicitations, tu as trouvé l'Easter Egg du Grand Sud !",
    body: [
      {
        icon: '🏉',
        title: 'Le calcul est simple',
        text: "On a tellement d'étoiles sur le maillot qu'on commence à sérieusement concurrencer la Voie Lactée. À ce rythme, la NASA va bientôt nous demander des conseils de navigation.",
      },
      {
        icon: '📊',
        title: 'La règle des 80-20',
        text: "80% du temps, on joue un rugby de rêve. Les 20% restants, on attend juste que l'adversaire finisse de croire qu'il a une chance.",
      },
      {
        icon: '⚡',
        title: "L'effet Antoine Dupont",
        text: "Scientifiquement, il est prouvé qu'Antoine peut être à trois endroits différents du terrain au même moment. C'est du rugby quantique, tout simplement.",
      },
      {
        icon: '🍫',
        title: 'Le régime "Chocolatine"',
        text: "C'est le secret de notre puissance. On ne gagne pas des Brennus avec des pains au chocolat, restons sérieux deux minutes.",
      },
    ],
    conclusion:
      'Bref, le Stade ne perd jamais. Soit il gagne, soit il apprend... mais généralement, il gagne.',
    ps: "P.S. : Si tu es supporter d'un autre club, sache que cette application t'aime quand même, mais elle tourne 15% moins vite pour toi.",
    toggle: 'EN',
  },
  en: {
    alert: '🔴⚫ Alert: Scientific Truth (slightly biased)',
    subtitle: 'Congratulations, you found the Grand Sud Easter Egg!',
    body: [
      {
        icon: '🏉',
        title: 'The math is simple',
        text: "We have so many stars on our jersey that we're seriously starting to rival the Milky Way. At this rate, NASA will soon be asking us for navigation tips.",
      },
      {
        icon: '📊',
        title: 'The 80-20 rule',
        text: "80% of the time, we play dream rugby. The other 20%, we're just waiting for the opponent to finish believing they have a chance.",
      },
      {
        icon: '⚡',
        title: 'The Antoine Dupont effect',
        text: "Scientifically proven — Antoine can be in three different spots on the pitch simultaneously. It's quantum rugby, plain and simple.",
      },
      {
        icon: '🍫',
        title: 'The "Chocolatine" diet',
        text: "That's the secret to our power. You don't win Brennus trophies on pain au chocolat, let's be serious for two seconds.",
      },
    ],
    conclusion:
...
// ── Component ───────────────────────────────────────────────────────────────────
export const StadeToulousainOverlay = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        background:
          'radial-gradient(ellipse at 50% 40%, rgba(230,50,40,0.18) 0%, rgba(26,26,26,0.97) 65%)',
      }}
    >
      {/* Grain overlay */}
      <div
      ...
      <div
        ...
        style={{
          ...
          animation: visible ? 'st-scan 3s ease-in-out infinite' : 'none',
        }}
      />

      <style>{`
        @keyframes st-logo-in {
          0%   { transform: translateY(-60px) scale(0.7); opacity: 0; filter: brightness(3); }
          60%  { transform: translateY(8px) scale(1.04); opacity: 1; filter: brightness(1.2); }
        ...
        @keyframes st-pulse {
          0%, 100% { box-shadow: 0 0 40px rgba(230,50,40,0.2), inset 0 0 30px rgba(230,50,40,0.04); }
          50%       { box-shadow: 0 0 80px rgba(230,50,40,0.45), inset 0 0 60px rgba(230,50,40,0.08); }
        }
      `}</style>
      ...
      <div
        ...
        style={{
          background: '#E63228',
          animation: visible ? 'st-flash 0.8s ease-out both' : 'none',
        }}
      />

      {/* Main card */}
      <div
        className="relative mx-4 flex w-full max-w-xl flex-col overflow-hidden rounded-3xl"
        ...
        style={{
          ...
          maxHeight: '90vh',
          animation: visible
            ? 'st-pulse 3s ease-in-out 0.5s infinite, st-card-in 0.5s cubic-bezier(0.22,1,0.36,1) 0.1s both'
            : 'none',
        }}
      >
        {/* Lang toggle */}
        <button
          ...
          className="absolute top-4 right-4 z-10 rounded-full px-3 py-1 font-mono text-xs font-bold transition-all hover:brightness-125"
          style={{
            background: 'rgba(230,50,40,0.12)',
            border: '1px solid rgba(230,50,40,0.4)',
            color: '#E63228',
          }}
        >
          {t.toggle}
        ...
        {/* Logo */}
        <div
          className="flex items-center justify-center pt-8 pb-3"
          style={{
            animation: visible ? 'st-logo-in 0.7s cubic-bezier(0.22,1,0.36,1) 0.15s both' : 'none',
          }}
        >
          <div
            style={{
        ...
        {/* Scrollable content */}
        <div
          className="rs-scrollbar flex-1 overflow-y-auto px-6 py-4"
        ...
        >
          ...
          <p className="mb-5 text-center text-sm font-medium text-white/80">{t.subtitle}</p>
            ...
            {t.body.map((item) => (
              <div
                ...
                style={{
                  ...
                  border: '1px solid rgba(230,50,40,0.15)',
                }}
              >
                <p className="mb-0.5 text-xs font-bold" style={{ color: '#E63228' }}>
        ...
        <div className="h-1 w-full" style={{ background: 'rgba(255,255,255,0.05)' }}>
          <div
            ...
            style={{
              width: `${progress}%`,
              background: 'linear-gradient(90deg, #8B0000, #E63228, #ff7a6a)',
