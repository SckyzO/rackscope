...
export interface AccentMeta   { id: AccentColor;           label: string; hex: string }
export interface PaletteMeta  { id: LightTheme | DarkTheme; label: string; desc: string; preview: { bg: string; surface: string; border: string } }
...
export const LIGHT_THEMES: PaletteMeta[] = [
  { id: 'slate', label: 'Slate',    desc: 'Clean neutral white',  preview: { bg: '#f9fafb', surface: '#ffffff', border: '#e5e7eb' } },
  { id: 'warm',  label: 'Warm',     desc: 'Soft cream tones',     preview: { bg: '#faf9f7', surface: '#fffdf9', border: '#e8e4de' } },
  { id: 'cool',  label: 'Cool',     desc: 'Crisp blue-gray',      preview: { bg: '#f8fafc', surface: '#ffffff', border: '#e2e8f0' } },
  { id: 'solarized', label: 'Solarized', desc: 'Easy on the eyes ☀️', preview: { bg: '#eee8d5', surface: '#fdf6e3', border: '#cfc8b0' } },
];

export const DARK_THEMES: PaletteMeta[] = [
  { id: 'void',   label: 'Void',   desc: 'Deep pure black',       preview: { bg: '#030712', surface: '#111827', border: '#1f2937' } },
  { id: 'navy',   label: 'Navy',   desc: 'Dark slate blue',       preview: { bg: '#020617', surface: '#0f172a', border: '#1e293b' } },
  { id: 'forest', label: 'Forest', desc: 'Dark zinc green',       preview: { bg: '#09090b', surface: '#18181b', border: '#27272a' } },
  { id: 'matrix', label: 'Matrix', desc: 'There is no spoon 🟩',  preview: { bg: '#000800', surface: '#001200', border: '#003000' } },
];
...
const ACCENT_PALETTES: Record<AccentColor, Record<string, string>> = {
  indigo: {
    ...
    '--color-accent': '#465fff', '--shadow-focus-ring': '0px 0px 0px 4px rgb(70 95 255 / 12%)',
  },
  violet: {
    '--color-brand-25': '#faf5ff', '--color-brand-50': '#f5f3ff', '--color-brand-100': '#ede9fe',
    '--color-brand-200': '#ddd6fe', '--color-brand-300': '#c4b5fd', '--color-brand-400': '#a78bfa',
    '--color-brand-500': '#7c3aed', '--color-brand-600': '#6d28d9', '--color-brand-700': '#5b21b6',
    '--color-brand-800': '#4c1d95', '--color-brand-900': '#3b1578', '--color-brand-950': '#2e1065',
    '--color-accent': '#7c3aed', '--shadow-focus-ring': '0px 0px 0px 4px rgb(124 58 237 / 12%)',
  },
  emerald: {
    '--color-brand-25': '#f6fef9', '--color-brand-50': '#ecfdf5', '--color-brand-100': '#d1fae5',
    '--color-brand-200': '#a7f3d0', '--color-brand-300': '#6ee7b7', '--color-brand-400': '#34d399',
    '--color-brand-500': '#059669', '--color-brand-600': '#047857', '--color-brand-700': '#065f46',
    '--color-brand-800': '#064e3b', '--color-brand-900': '#022c22', '--color-brand-950': '#011a14',
    '--color-accent': '#059669', '--shadow-focus-ring': '0px 0px 0px 4px rgb(5 150 105 / 12%)',
  },
  rose: {
    '--color-brand-25': '#fff5f7', '--color-brand-50': '#fff1f2', '--color-brand-100': '#ffe4e6',
    '--color-brand-200': '#fecdd3', '--color-brand-300': '#fda4af', '--color-brand-400': '#fb7185',
    '--color-brand-500': '#e11d48', '--color-brand-600': '#be123c', '--color-brand-700': '#9f1239',
    '--color-brand-800': '#881337', '--color-brand-900': '#6c1029', '--color-brand-950': '#4c0519',
    '--color-accent': '#e11d48', '--shadow-focus-ring': '0px 0px 0px 4px rgb(225 29 72 / 12%)',
  },
  amber: {
    '--color-brand-25': '#fffdf5', '--color-brand-50': '#fffbeb', '--color-brand-100': '#fef3c7',
    '--color-brand-200': '#fde68a', '--color-brand-300': '#fcd34d', '--color-brand-400': '#fbbf24',
    '--color-brand-500': '#d97706', '--color-brand-600': '#b45309', '--color-brand-700': '#92400e',
    '--color-brand-800': '#78350f', '--color-brand-900': '#5a2800', '--color-brand-950': '#3d1a00',
    '--color-accent': '#d97706', '--shadow-focus-ring': '0px 0px 0px 4px rgb(217 119 6 / 12%)',
  },
};

const LIGHT_PALETTES: Record<LightTheme, Record<string, string>> = {
  slate:     { '--color-white': '#ffffff', '--color-gray-50': '#f9fafb', '--color-gray-100': '#f3f4f6', '--color-gray-200': '#e5e7eb' },
  warm:      { '--color-white': '#fffdf9', '--color-gray-50': '#faf9f7', '--color-gray-100': '#f4f2ef', '--color-gray-200': '#e8e4de' },
  cool:      { '--color-white': '#ffffff', '--color-gray-50': '#f8fafc', '--color-gray-100': '#f1f5f9', '--color-gray-200': '#e2e8f0' },
  ...
  solarized: { '--color-white': '#fdf6e3', '--color-gray-50': '#eee8d5', '--color-gray-100': '#e3dcc8', '--color-gray-200': '#cfc8b0' },
};

const DARK_PALETTES: Record<DarkTheme, Record<string, string>> = {
  void:   { '--color-gray-dark': '#111827', '--color-gray-700': '#374151', '--color-gray-800': '#1f2937', '--color-gray-900': '#111827', '--color-gray-950': '#030712' },
  navy:   { '--color-gray-dark': '#0f172a', '--color-gray-700': '#334155', '--color-gray-800': '#1e293b', '--color-gray-900': '#0f172a', '--color-gray-950': '#020617' },
  forest: { '--color-gray-dark': '#18181b', '--color-gray-700': '#3f3f46', '--color-gray-800': '#27272a', '--color-gray-900': '#18181b', '--color-gray-950': '#09090b' },
...
export const ThemeProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (isMatrix) {
      ...
      ['--color-gray-300', '--color-gray-400', '--color-gray-500', '--color-gray-600']
  ...
  const setLightTheme = (t: LightTheme) => { setLightThemeState(t); setMode('light'); };
  // Clicking a dark theme → auto-switch to dark mode so user can see it
  const setDarkTheme  = (t: DarkTheme)  => { setDarkThemeState(t);  setMode('dark');  };
